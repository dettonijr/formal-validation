...
% After this, A and B share the key Kab
%
% This code was highly based in the following paper:
...
fof(server_keys_cant_be_used_as_nonces, axiom, ( ![X] : (
    ~nonce(kt(X))   
))).

fof(nonces_cant_be_used_as_keys, axiom, (![X] : (
    ~(iskey(X) & nonce(X))    
))).

fof(all_kt_are_keys, axiom, ( ![X] : (
    iskey(kt(X))
))).
...
fof(b_receives_the_last_message_and_decrypt, axiom, (
    ![XA,XK,XNA] : (
    (sent(XA,b,pair(encr(triple(XA,XK,tb(XNA)),bt),encr(nb(XNA),XK))) & sb(pair(XA,XNA)) & iskey(XK))
    =>
    bk(key(XK,XA))
    )
)).
...
fof(ni_is_fresh_to_attacker, axiom, (
    if(ni)
)).

fof(what_is_fresh_to_attacker_is_fresh_to_b, axiom, ( ![X] : (
    if(X) => (bf(X) & im(X))
))).
